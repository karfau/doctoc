# About the fork DocToc-Explicit

## Explicit vs. Implicit (difference from main repo)

**This fork/branch prefers explicit over implicit behaviour, which is a breaking change compared to https://github.com/thlorenz/doctoc**

Meaning:
1. If your markdown files do not contain generated TOCs, running `doctoc .` doesn't do anything.
2. Running `doctoc SomeFile.md` does the same thing as in the main repo, it prepends the generated TOC.
3. Running `doctoc .` on a directory that already contains markdown files with a generated TOC, 
   will only update those.
4. Running `doctoc --all .` will add tocs to all markdown files that contain headers (the default behaviour of `doctoc .` in the main repo)

Currently I have not added tests for the change in behaviour, 
but **all existing tests still pass without modification** 
and I have tested the changed behaviour manually.

# Original DocToc Readme 
**(just without badges, so not reflecting the current state of the code in this repo)** 
